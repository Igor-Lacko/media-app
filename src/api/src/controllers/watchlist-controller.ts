 ...
 */
function CalculateTvShowProgress(
	tvShow: DBTvShow,
	inEpisodes: boolean,
): { progress: string; progressPercentage: number } {
	...
	const progressPercentageEpisodes =
		(completedEpisodes / Math.max(totalEpisodes, 1)) * 100;
	...
	const progressPercentageSeasons =
		(completedSeasons / Math.max(totalSeasons, 1)) * 100;
 ...
 */
function CalculateCourseProgress(course: DBCourse): {
	progress: string;
...
function TvShowToWatchListItem(
	tvShow: DBTvShow,
	progressInEpisodes?: boolean,
): WatchListItem {
	return {
		title: tvShow.title,
 ...
 */
export default async function GetWatchlistItems(
	...
	try {
		...
		return {
			entertainment: [...moviesToWatch, ...tvShowsToWatch],
