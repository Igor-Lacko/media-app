 ...
 */
export default function useGenreDropdown<T extends { genres?: Genre[] }>(
	...
	const props: DropdownProps = {
		...
		options: Object.values(Genre)
			.filter((value: Genre) => value !== Genre.ALL)
		...
		onChange(value: Genre | SortKey) {
			...
			ref.current.genres?.includes(value as Genre) ?
				// Yes, remove it
				(ref.current.genres = ref.current.genres?.filter(
					(genre) => genre !== value,
				))
			:	// No, add it
		...
		// Initial genres
		initialSelections: initial.filter((genre) => genre !== Genre.ALL),
	};
