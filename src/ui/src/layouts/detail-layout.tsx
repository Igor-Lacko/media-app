 ...
 */
export default function DetailLayout<T extends DetailFillable>(
	props: DetailProps<T>,
) {
	...
	const editBarProps: EditBarProps = {
		...
		onAdd:
			props.addTitle ?
				() => navigate("add", { state: { id: props.model.identifier } })
			:	undefined,
		...
		onEdit:
			props.editTitle ?
				() =>
					navigate("edit", { state: { id: props.model.identifier } })
			:	undefined,
		...
		onSetWatchStatus:
			props.watchStatus ?
				() => setVisibleModal(VisibleModal.WATCH_STATUS)
			:	undefined,

		// Set description
		onSetDescription:
			props.setDescriptionFunction ?
				() => setVisibleModal(VisibleModal.DESCRIPTION)
			:	undefined,

		// Mark submedia as completed
		markSubmediaAsCompletedTitle: props.markSubmediaAsCompletedTitle,
		onCompleteSubmedia:
			props.completeSubmediaFunction ?
				() => setVisibleModal(VisibleModal.MARK_SUBMEDIA_AS_COMPLETED)
			:	undefined,

		// Open notes modal
		onAddNote:
			props.addNoteFunction ?
				() => setVisibleModal(VisibleModal.ADD_NOTE)
			:	undefined,
	};
	...
	return (
		<div
			className={
				"flex flex-col w-full max-h-screen items-center justify-start overflow-x-hidden\
                    overflow-y-hidden bg-white dark:bg-gray-800"
			}
		>
			<div
				className={
					"flex items-center justify-start ml-10 mt-5 w-full h-1/25 sm:h-1/20"
				}
			>
				<FaArrowLeft
			...
			{props.headerType === DetailHeaders.ENTERTAINMENT && (
				<EntertainmentDetailHeader {...props} />
			)}
			{props.headerType === DetailHeaders.LECTURE && (
				<LectureDetailHeader
					{...props}
					model={props.model as unknown as Lecture}
				/>
			)}
			{props.headerType === DetailHeaders.COURSE && (
				<CourseDetailHeader
					{...props}
					model={props.model as unknown as Course}
				/>
			)}
			{props.headerType === DetailHeaders.SEASON && (
				<SeasonDetailHeader
					{...props}
					model={props.model as unknown as Season}
				/>
			)}
			{props.headerType === DetailHeaders.EPISODE && (
				<EpisodeDetailHeader
					{...props}
					model={props.model as unknown as Episode}
				/>
			)}
			<EditBar {...editBarProps} />
			{/** Submedia list */}
			{props.listProps && <MediaItemList {...props.listProps} />}
			...
			{/** Modals */}
			{/** 1. Delete modal */}
			{visibleModal === VisibleModal.DELETE && (
				<ConfirmModal
			...
			{/** 2. Rate modal */}
			{visibleModal === VisibleModal.RATE
				&& props.rateFunction !== undefined
				&& props.rateFunction !== null && (
					<SliderModal
						title={props.rateTitle || "Rate"}
						onSliderEnter={async (rating: number) => {
							props.rateFunction
								&& (await props.rateFunction(rating));
							setVisibleModal(VisibleModal.NONE);
						}}
						initialValue={
							props.rating && props.rating > 0 ? props.rating : 0
						}
						maxValue={10}
						onClose={() => setVisibleModal(VisibleModal.NONE)}
					/>
			...
			{visibleModal === VisibleModal.WATCH_STATUS
				&& props.watchStatus && (
					<EnumModal<WatchStatus>
						title={"Select Watch Status"}
						selectOptions={props.watchStatusOptions!.map(
							(status) => {
								return {
						...
						onSelect={async (watchStatus: WatchStatus) => {
							props.watchStatusFunction
								&& (await props.watchStatusFunction(
									watchStatus,
								));
							setVisibleModal(VisibleModal.NONE);
						}}
			...
			{visibleModal === VisibleModal.DESCRIPTION && (
				<TextAreaModal
					title={"Set Description"}
					initialText={props.description}
					onSetText={async (description: string) => {
						props.setDescriptionFunction
							&& (await props.setDescriptionFunction(
								description,
							));
						setVisibleModal(VisibleModal.NONE);
					}}
			...
			{visibleModal === VisibleModal.PLAY_NOFILE && (
				<FileBrowseModal
					...
					onSetText={async (videoUrl: string) => {
						props.setVideoUrlFunction
							&& (await props.setVideoUrlFunction(videoUrl));
						setVisibleModal(VisibleModal.NONE);
					}}
					onClose={() => setVisibleModal(VisibleModal.NONE)}
				/>
			...
			{visibleModal === VisibleModal.PLAY_WRONG_FILE && (
				<FileBrowseModal
			...
			{/** 7. Add note modal */}
			{visibleModal === VisibleModal.ADD_NOTE
				&& props.addNoteFunction && (
					<MarkdownModal
						title={"Add Note"}
						onSetText={async (note: string) => {
							props.addNoteFunction
								&& (await props.addNoteFunction({
									content: note,
								}));
							setVisibleModal(VisibleModal.NONE);
						}}
			...
			{visibleModal === VisibleModal.MARK_SUBMEDIA_AS_COMPLETED
				&& props.completeSubmediaFunction && (
					<SliderModal
						title={props.markSubmediaAsCompletedTitle || ""}
						message={
							"This will mark the first X submedia of this season as completed\
                        and the rest as not watched."
						}
						onSliderEnter={async (count: number) => {
							props.completeSubmediaFunction
								&& (await props.completeSubmediaFunction(
									(props.model as unknown as Season)
										.identifier!,
									count,
								));
							setVisibleModal(VisibleModal.NONE);
						...
						initialValue={
							props.submedia?.filter(
								(item) =>
									item.watchStatus === WatchStatus.COMPLETED,
							).length || 0
						}
