...
export default function ApiFormLayout(props: ApiFormProps) {
	...
	return (
		...
		>
			...
			<div
				className={
					"flex items-center justify-start ml-20 mt-5 w-full h-1/25 sm:h-1/20"
				}
			>
				<FaArrowLeft
			...
			<div className={"flex items-center justify-center w-full h-24/25"}>
				<div
					className={
						"flex flex-col w-200 h-auto items-center justify-start\
				...
				>
					...
					<div
						className={
							"flex flex-col items-center justify-start w-full mb-1"
						}
					>
						<span className={"text-red-500 text-sm min-h-6"}>
							{error ?
								"At least one of Title or IMDb Id must be provided."
							:	""}
						</span>
						{/** Title */}
						<input
						...
						<input
							...
							onChange={(e) =>
								(imdbIdRef.current = e.target.value)
							}
							onClick={() => setError(false)}
						/>
					</div>
					...
					>
						<RoundedButton
							...
							onClick={async () => {
								...
								// Submit
								await props
									.onSubmit(trimmedTitle, trimmedImdbId)
									.then((res) => {
										setLoading(false);
										res.success ?
