...
export default function Watchlist(props: WatchlistProps) {
	...
	const { thumbnails: validThumbnails, setThumbnails } = useThumbnail(
		props.items,
	);
	...
	return (
		<div
			className={
				"flex flex-col w-full h-auto overflow-y-auto "
				...
				>
					...
					{validThumbnails[index] ?
						<img
							...
							alt={item.title}
							className={
								"w-1/10 size-32 mr-4 p-2 rounded-lg object-cover shadow-md dark:shadow-lg"
							}
						/>
					:	<div
							className={
								"w-1/12 size-32 bg-gray-200 dark:bg-gray-700 flex\
					...
					<h2
						className={
							"text-gray-800 dark:text-gray-400 text-lg font-semibold font-[Roboto] hover:underline cursor-pointer"
						}
					...
					{props.showShortDescription && item.shortDescription && (
						<p
							className={
								"text-gray-600 dark:text-gray-400 text-sm mt-1 absolute left-2/5 max-w-2/5 wrap-anywhere line-clamp-3"
							}
						>
