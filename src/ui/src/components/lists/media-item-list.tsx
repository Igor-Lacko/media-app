	...
	if (props.items.length === 0) {
		return (
			<ItemNotFound
				...
				message={
					props.notFoundMessage || "There are no items to display."
				}
			/>
		);
	}
			...
			{props.items.map((item, index) => (
				...
				>
					{props.showThumbnail
						...
						:	<div
								className={
									"size-32 rounded-lg bg-gray-200 dark:bg-gray-700\
					...
					>
						<h3
							className={
								"text-lg font-semibold text-gray-900 dark:text-gray-400"
							}
						>
							{item.title}
						</h3>
					...
					<div
						className={
							"flex h-full w-1/10 items-center justify-center"
						}
					>
						{item.genres && (
							<span
								className={
									"text-md text-gray-500 dark:text-gray-300"
								}
							>
								{item.genres
					...
					<div
						className={
							"flex flex-col h-full w-1/10 items-center justify-center"
						}
					>
						{props.showRating && (
