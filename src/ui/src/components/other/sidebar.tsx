...
export default function Sidebar({
	...
	return (
		...
		>
			<div
				className={
					"flex w-full flex-col items-center justify-start space-y-2 px-4 pt-5 pb-5 sm:pt-10 border-b-2 border-b-gray-300 dark:border-b-gray-700 mb-2 h-auto"
				}
			>
				<div className={"flex items-center justify-end w-full"}>
					<FaVideo
						className={
							"text-purple-500 lg:text-7xl w-full md:text-2xl sm:text-2xl font-bold justify-center"
						}
					/>
				</div>
				<span
					className={
						"text-purple-500 lg:text-3xl md:text-lg sm:text-lg font-bold font-[lobster]"
					}
				>
					Media Player
				</span>
			...
			<div className={"flex flex-col items-center p-2 w-full h-auto"}>
				<SidebarButton label={"Home"} icon={<FaHome />} link={"/"} />
				<SidebarButton
					label={"Movies"}
					icon={<FaFilm />}
					link={"/movies"}
			...
			<button
				className={
					"fixed bottom-0 left-0 flex w-full items-center h-10 justify-center cursor-pointer hover:bg-gray-300 dark:hover:bg-gray-500\
