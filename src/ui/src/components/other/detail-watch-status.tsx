...
import { FaCheckCircle, FaPlayCircle, FaPauseCircle, FaClock } from "react-icons/fa";
...
export default function DetailWatchStatus({ watchStatus, extraClassNames }: { watchStatus: WatchStatus, extraClassNames?: string }) {
    const statusIcon = 
        watchStatus === WatchStatus.COMPLETED ? <FaCheckCircle className="text-green-500 text-2xl" /> :
        watchStatus === WatchStatus.WATCHING ? <FaPlayCircle className="text-blue-500 text-2xl" /> :
        watchStatus === WatchStatus.NOT_WATCHED ? <FaPauseCircle className="text-gray-500 text-2xl" /> :
        watchStatus === WatchStatus.PLAN_TO_WATCH ? <FaClock className="text-yellow-500 text-2xl" /> :
        null;
