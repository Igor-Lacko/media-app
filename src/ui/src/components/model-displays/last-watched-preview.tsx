...
export default function LastWatchedPreview(props: CarouselProps) {
			...
			const thumbnails = await Promise.all(
				props.models.map(async (model) => {
					if (model.thumbnailUrl) {
						return (
							(await IsValidFile(model.thumbnailUrl))
	...
	return (
		...
		>
			...
			<div
				className={
					"w-full h-3/4 flex-grow flex items-center justify-center relative border-b-2 border-black dark:border-gray-700"
				}
			>
				{validThumbnails[currentIndex] ?
					<img
						src={
							ImagePathToURL(
								props.models[currentIndex].thumbnailUrl,
							).path
						}
				...
				<FaPlay
					...
					onClick={() =>
						navigate(`${props.models[currentIndex].url}`)
					}
				/>
			</div>
			{/** Rest of the data */}
			<div
				className={
					"flex flex-col items-start justify-start w-full p-2 text-black dark:text-gray-400"
				}
			>
				...
				<div className={"flex items-center justify-between w-full p-2"}>
					<h2
						className={
							"text-xl font-bold cursor-pointer hover:underline"
						}
						onClick={() => navigate(props.models[currentIndex].url)}
					>
						{props.models[currentIndex].title}
					</h2>
					{props.models[currentIndex].length && (
						<span
							className={
				...
				<div className={"flex items-center justify-start w-full p-2"}>
					...
					{props.models[currentIndex].subTitle && (
						<span
							className={
								"text-md text-gray-500 dark:text-gray-400 italic"
							}
						>
							{props.models[currentIndex].subTitle}
						</span>
					...
					>
						{props.models.map((_model, index) => (
							<button
								...
								className={
									"text-md text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-gray-200\
