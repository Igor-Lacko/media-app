...
export default function ControlBar(props: ControlBarProps) {
	...
	const spanClasses =
		"flex items-end text-lg p-2 rounded-full text-gray-500\
	...
	const sortProps: DropdownProps = {
		...
		options: props.sortOptions.map((key: SortKey) => {
			return SortKeyAdapter(key);
		}),
	...
	const filterProps: DropdownProps = {
		width: dropdownWidth,
		icon: <FaSort className={"text-gray-500 h-4 w-4"} />,
		options: Object.values(Genre).map((value) => {
			return GenreAdapter(value);
		}),
	...
	return (
		<div
			className={
				"flex items-center w-full h-1/10 justify-center p-5 border-b-2 border-gray-300 dark:border-gray-700"
			}
		>
			<span
				className={
					"text-4xl sm:text-lg md:text-2xl font-semibold h-full w-2/10 text-black dark:text-gray-200 ml-10"
				}
			>
				{props.title}
			</span>
			<div
				className={
					"flex w-8/10 h-full items-center space-x-10 justify-end mr-10"
				}
			>
				<input
					type="text"
					placeholder="Search..."
					className={
						"flex w-2/10 mt-2.5 h-9/10 p-2 rounded-lg border border-gray-300 dark:border-gray-700 text-gray-700 \
				...
				<div
					className={
						"flex flex-col space-x-2 items-center justify-center h-full w-auto ml-10"
					}
				>
					<span className={"text-gray-500 font-light text-sm mb-1"}>
				...
				<DropdownMenu {...sortProps} />
				{props.filter && <DropdownMenu {...filterProps} />}
