...
export default function SeasonDetailHeader(props: DetailProps<Season>) {
	return (
		<div
			className={
				"flex items-center justify-start w-full h-auto max-h-80 px-10 mt-5"
			}
		>
			{/** Title, nofepisodes */}
			<div
				className={
					"flex flex-col pb-5 w-3/10 items-start justify-between h-full text-black dark:text-gray-400"
				}
			>
				<div>
					<h1
			...
			<div
				className={
					"flex flex-col items-center justify-start w-7/10 ml-10 h-full"
				}
			>
				<p
					className={
				...
				>
					{props.description
						|| "No description seems to be here yet. You can add one by clicking on the button below :))."}
				</p>
			</div>
		</div>
	);
