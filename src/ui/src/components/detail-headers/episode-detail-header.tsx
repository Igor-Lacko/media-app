...
export default function EpisodeDetailHeader(props: DetailProps<Episode>) {
	return (
		<div
			className={
				"flex items-center justify-start w-full h-auto max-h-80 px-10"
			}
		>
			{/** Title, length, rating */}
			<div
				className={
					"flex flex-col pb-5 w-3/10 items-start justify-start h-full space-y-2 text-black dark:text-gray-400"
				}
			>
				<h1
					className={
			...
			<div
				className={
					"flex flex-col items-center justify-start w-6/10 ml-10 h-full"
				}
			>
				<p
					className={
				...
				>
					{props.model.shortDescription
						|| "No description seems to be here yet. You can add one in the edit episode form."}
				</p>
			</div>
			{/** Watch status, play button */}
			<div
				className={
					"flex flex-col items-center justify-center ml-10 w-1/10 h-full space-y-5"
				}
			>
