 ...
 */
export default function EntertainmentDetailHeader<T extends DetailFillable>(
	props: DetailProps<T>,
) {
	...
	return (
		<div
			className={
				"flex items-center justify-start w-full h-auto max-h-80 px-5"
			}
		>
			{/*  thumbnail */}
			{props.hasThumbnail && (
				<div
					className={
						"flex flex-col items-center justify-center max-w-4/10 pb-5 h-full"
					}
				>
					{thumbnailLoaded ?
						<DetailImage src={props.model.thumbnailUrl!} />
					:	<ItemNotFound
							...
							extraClassNames={
								"rounded-lg shadow-md ml-10 border-2 border-black dark:border-gray-700"
							}
						/>
					}
				</div>
			...
			<div
				className={
					"flex flex-col py-5 w-2/10 items-start justify-start ml-15 h-full text-black dark:text-gray-400"
				}
			>
				<h1
					className={
				...
				<div
					className={
						"flex flex-col text-lg items-start justify-end py-5 h-full w-full"
					}
				>
					{/** Either length or nof seasons/episodes */}
					{props.playTitle
						&& props.model.length !== undefined
						&& props.model.length !== null && (
							<span
								className={
									"text-gray-500 dark:text-gray-400 text-md font-medium italic"
								}
							>
					...
					{props.hasGenres && (
						<div className="text-lg text-gray-500 dark:text-gray-400 italic">
							{props.model
								.genres!.filter((genre) => genre !== Genre.ALL)
								.map((genre) => GenreAdapter(genre).key)
								.join(", ")}
						</div>
					)}
				</div>
			...
			<div
				className={
					"flex flex-col py-5 w-4/10 ml-10 items-start overflow-y-auto justify-start h-full space-y-2 text-gray-400"
				}
			>
				<p
					className={
						"text-md flex h-full w-full text-black wrap-anywhere dark:text-gray-400"
					}
				>
			...
			<div
				className={
					"flex flex-col items-center mt-3 ml-10 justify-center py-5 w-2/10 h-full\
                        space-y-5"
				}
			>
				{props.rating !== undefined
					&& props.rating !== null
					&& props.rating !== -1 && (
						<HeaderRating rating={props.rating} />
					)}
				{props.watchStatus !== undefined && props.rating !== null && (
