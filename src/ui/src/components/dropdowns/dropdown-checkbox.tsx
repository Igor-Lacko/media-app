...
export default function DropdownCheckbox(props: DropdownProps) {
	...
	return (
		<div
			className={
				"relative flex flex-col items-start h-full " + props.width
			}
		>
			<span
			...
			>
				<span className="flex items-start w-1/2 sm:w-full sm:text-sm sm:p-0 md:text-base lg:text-lg">
					<div
						className={
							"flex items-center rounded-full lg:p-2 cursor-pointer hover:bg-gray-300 dark:hover:bg-gray-400\
                            transition-all duration-300 ease-in-out"
						}
						onClick={() => setIsOpen(!isOpen)}
					>
						{props.prefix ?
							`${props.prefix} ${props.initialText}` // Initial text constant for chexckboxes
						:	props.initialText}
						<div className={"ml-1"}>{props.icon}</div>
					</div>
				</span>
			</span>
			<div
				className={classNames(
					...
					{ "scale-y-0": !isOpen, "scale-y-100": isOpen },
				)}
			>
				<ul
					className={
						"flex flex-col p-2 max-h-60 overflow-y-auto text-gray-700 dark:text-gray-400"
					}
				>
