...
export default function SettingsPage() {
	...
	// Load DB modal
	const [loadDBModalVisible, setLoadDBModalVisible] = useState(false);

	// Deletion status message
	const [deletionStatusMessage, setDeletionStatusMessage] = useState("");

	// Key submission status message
	const [keySubmissionStatusMessage, setKeySubmissionStatusMessage] =
		useState("");

	// A db export status message
	const [exportStatusMessage, setExportStatusMessage] = useState("");

	// Load DB status message
	const [loadStatusMessage, setLoadStatusMessage] = useState("");

	// Used for all divs here
	const divClasses =
		"flex w-full items-center justify-between space-x-7 p-4 h-20\
	...
	return (
		<div
			className={
				"flex w-full h-full flex-col items-start justify-start p-4 m-0"
			}
		>
			<h1
				className={
					"text-4xl font-bold text-gray-800 dark:text-gray-400 ml-10 mb-10"
				}
			>
				Settings
			</h1>
			...
			<div className={divClasses}>
				<h2
					className={
						"text-lg font-semibold text-gray-800 dark:text-gray-400"
					}
				>
					Dark mode
				</h2>
			...
			<div className={divClasses}>
				...
				<Toggle
					...
					onChange={async (checked: boolean) => {
						const response =
							await ToggleTvShowProgressDisplay(checked);
						if (response) {
							setSettings({
								...settings,
								tvShowProgressInEpisodes: checked,
							});
						} else {
			...
			<div className={divClasses}>
				...
				<span className={"text-sm text-gray-500 dark:text-gray-400"}>
					If enabled, this shows a preview of the note in markdown
					editors for lecture notes.
				</span>
				<Toggle
			...
			<div className={divClasses}>
				...
				<span className={"text-sm text-gray-500 dark:text-gray-400"}>
					This enables showing external images fetched from the OMDb
					or TV Maze APIs (if present in the response) in addition to
					local files.
				</span>
				<Toggle
					checked={settings.showExternalImages}
					onChange={async (checked: boolean) =>
						await ToggleExternalImages(checked).then((response) => {
							if (response) {
								setSettings({
									...settings,
									showExternalImages: checked,
								});
							} else {
								alert(
									"Failed to update external images setting.",
								);
							}
						})
					}
			...
			<div className={divClasses}>
				...
				>
					...
					{!settings.hasApiKey && (
						<button
							className={
								"text-blue-500 dark:text-blue-400 hover:underline ml-1 cursor-pointer"
							}
							onClick={async () =>
				...
				{settings.hasApiKey ?
					<RoundedButton
						...
						extraClassNames={
							"bg-red-500 text-white hover:bg-red-400 dark:hover:bg-red-600 transition-all duration-300 ease-in-out"
						}
					/>
				:	<div className={"flex items-center justify-between h-full"}>
						...
						>
							{error && (
								<span
									className={
										"text-red-500 dark:text-red-400 text-xs mr-5"
									}
								>
							...
							<input
								...
								className={classNames(
									...
									{
										"ring-2 ring-red-500 dark:ring-red-400 outline-none":
											error,
									},
								)}
							/>
						...
						<RoundedButton
							...
							extraClassNames={
								"bg-blue-500 text-white hover:bg-blue-400 dark:hover:bg-blue-600\
			...
			<div className={divClasses}>
				<h2
					className={
						"text-lg font-semibold dark:text-gray-400 text-gray-800"
					}
				>
					Reset the DB
				</h2>
			...
			{/** Export and load DB button */}
			<div className={divClasses + "border-b"}>
				<h2
					className={
						"text-lg font-semibold dark:text-gray-400 text-gray-800"
					}
				...
				<div className={"flex space-x-4"}>
					...
					/>
					<RoundedButton
						onClick={() => setLoadDBModalVisible(true)}
						extraClassNames={"bg-green-500 h-9/10"}
						text={"Load from JSON"}
					/>
				</div>
			</div>
				...
				<ConfirmModal
					...
					onConfirm={async () => {
						...
						setSettings({
							...
							hasApiKey: false,
							tvShowProgressInEpisodes:
								settings.tvShowProgressInEpisodes,
				...
				<CheckMarkModal
					...
					onConfirm={async (options) => {
						...
						if (res.success) {
							...
							setExportStatusMessage(
								`Failed to export data: ${res.errorMessage}`,
							);
						}
					}}
				/>
