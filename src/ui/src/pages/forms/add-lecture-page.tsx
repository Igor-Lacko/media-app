...
export default function AddLecturePage({ route }: { route?: any }) {
	...
	const { model: lecture, isLoading } = useFetchById<Lecture>(
		"/api/lectures",
		"lectureId",
	);
	...
	const [initial, setInitial] = useState(
		lecture || { ...defaultLecture(-1, courseId) },
	);
	...
	return (
			...
			submitFunction={
				!creating ?
					async (lecture: Lecture) =>
						await SubmitLecture(lecture, true)
				:	async (lecture: Lecture) =>
						await SubmitLecture(lecture, false, courseId)
			}
		...
		>
			<FormSection title={"Lecture Information"}>
				<InputOption
					title={"Lecture Title*"}
