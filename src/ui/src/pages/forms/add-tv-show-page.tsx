...
export default function AddTvShowPage({ route }: { route?: any }) {
	...
	const [initial, setInitial] = useState(
		tvshow || structuredClone(defaultTvShow),
	);
	...
	const [episodes, setEpisodes] = useState<Episode[]>(
		initial.seasons.flatMap((season) => season.episodes),
	);
	...
	useEffect(() => {
		if (!tvshow) {
			...
			episodeCounterRef.current =
				tvshow.seasons.flatMap((season) => season.episodes).length + 1;
			seasonCounterRef.current = tvshow.seasons.length + 1;
			setSeasons(tvshow.seasons || []);
			setEpisodes(
				tvshow.seasons.flatMap((season) => season.episodes) || [],
			);
			setInitial(tvshow);
		}
	}, [tvshow, isLoading]);
	...
	// Props
	const genreDropdownProps = useGenreDropdown(
		tvShowRef,
		initial.genres || [],
	);
		...
		>
			<FormSection title={"Basic Information"}>
				...
				<TextAreaOption
					...
					onChange={(value) =>
						(tvShowRef.current.shortDescription = value)
					}
				/>
			</FormSection>
			<FormSection title={"Additional Information"}>
				...
				<FileBrowseOption
					...
					allowed={"image"}
					initial={
						ImagePathToURL(tvShowRef.current.thumbnailUrl).isLocal ?
							tvShowRef.current.thumbnailUrl || ""
						:	"External URL"
					}
					onChange={(value) =>
						(tvShowRef.current.thumbnailUrl = value)
					}
				/>
			</FormSection>
			<FormSection title={"Seasons"}>
				<AddOption
					...
					onChange={() => {
						setSeasons([
							...seasons,
							defaultSeason(seasonCounterRef.current++),
						]);
					}}
				/>
