...
export default function EpisodeVideo() {
	const { model: episode, isLoading: episodeLoading } = useFetchById<Episode>(
		"/api/episodes",
		"episodeId",
	)!;
	...
	const { model: show, isLoading: showLoading } =
	...
	if (episodeLoading || seasonLoading || showLoading) {
		...
		return <NotFoundPage message={"Episode, season, or show not found"} />;
	}
	...
	return (
		<VideoPlayerLayout
			...
			saveContinueAt={async (time: number) => {
				await UpdatePlaybackPosition<Episode>(
					"/api/episodes",
					episode,
					time,
				);
			...
			onInit={async () =>
				await UpdateData<TvShow>("/api/shows", show.identifier!, {
					watchStatus: WatchStatus.WATCHING,
				})
			}
			onFinish={async () =>
				await UpdateData<Episode>(
					"/api/episodes",
					episode.identifier!,
