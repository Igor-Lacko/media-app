...
export default function SeasonDetail() {
	const { model: season, isLoading: seasonLoading } = useFetchById<Season>(
		"/api/seasons",
		"seasonId",
	);

	// Tv show for url
	const { model: tvShow, isLoading: tvShowLoading } =
		useFetchById<TvShow>("/api/shows");
	...
	useEffect(() => {
		if (season) {
			...
			setEpisodesCompleted(
				season.episodes.map(
					(episode: Episode) =>
						episode.watchStatus === WatchStatus.COMPLETED,
				),
			);
		}
	}, [season]);
	...
	const props: DetailProps<Season> = {
		...
		listProps: {
			...
			notFoundMessage:
				"This season has no episodes yet. You can add one by clicking the 'Add Episode' button above, or edit the season to add an episode.",
		},
		addTitle: "Add Episode",
		editTitle: "Edit Season",
		deleteTitle: "Delete Season",
		deleteFunction: async () =>
			await DeleteData("/api/seasons", season.identifier!),
		...
		completeSubmediaFunction: async (id: number, count: number) => {
			const completed = Array(count)
				.fill(true)
				.concat(Array(season.episodes.length - count).fill(false));
			setEpisodesCompleted(completed);
			return await MarkSubmediaAsCompleted(
				`/api/seasons/${id}/mark-completed`,
				count,
			);
		},
		setDescriptionFunction: async (description: string) => {
			setDescription(description);
			return await UpdateDescription<Season>(
				"/api/seasons",
				season,
				description,
			);
