...
export default function TvShowDetail() {
	...
	const [watchStatus, setWatchStatus] = useState(
		tvShow?.watchStatus || WatchStatus.NOT_WATCHED,
	);
	...
	const props: DetailProps<TvShow> = {
		...
		listProps: {
			...
			notFoundMessage:
				"This TV show has no seasons yet. You can add one by clicking the 'Add Season' button above, or edit the TV show to add a season.",
		},
		watchStatusOptions: [
		...
		deleteFunction: async () =>
			await DeleteData("/api/shows", tvShow.identifier!),
		...
		watchStatusFunction: async (watchStatus) => {
			...
			return await UpdateWatchStatus<TvShow>(
				"/api/shows",
				tvShow,
				watchStatus,
			);
		...
		setDescriptionFunction: async (description: string) => {
			...
			return await UpdateDescription<TvShow>(
				"/api/shows",
				tvShow,
				description,
			);
