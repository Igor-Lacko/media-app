...
export default function EpisodeDetail() {
	...
	const { model: season, isLoading: seasonLoading } = useFetchById<Episode>(
		"/api/seasons",
		"seasonId",
	);
	...
	const props: DetailProps<Episode> = {
		...
		deleteFunction: async () =>
			await DeleteData("/api/episodes", episode.identifier!),
		rateFunction: async (rating: number) => {
			setRating(rating);
			return await UpdateRating<Episode>(
				"/api/episodes",
				episode,
				rating,
			);
		...
		setVideoUrlFunction: async (videoUrl: string) => {
			...
			return await UpdateVideoUrl<Episode>(
				"/api/episodes",
				episode,
				videoUrl,
			);
		...
		watchStatusFunction: async (watchStatus: string) => {
			...
			return await UpdateWatchStatus<Episode>(
				"/api/episodes",
				episode,
				watchStatus,
			);
